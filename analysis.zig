...
test "chi-squared" {
    ...
    var chi = chiSquared(1);
    chi.safe(false);

    const fit_eng = chi.calc(s_eng);
    const fit_not = chi.calc(s_not);
...
test "chi-squared safe" {
    const s_eng = try std.testing.allocator.dupe(u8, "Defend the east wall of the castle!");
    defer std.testing.allocator.free(s_eng);

    // TODO: used ccrypt.cipher.caesar to encrypt the text
    const s_not = try std.testing.allocator.dupe(u8, "Ghihqg wkh hdvw zdoo ri wkh fdvwoh!");
    defer std.testing.allocator.free(s_not);
...
test "bigram fitness" {
    ...
    const fit_eng = bi.calc(s_eng);
    const fit_not = bi.calc(s_not);
...
test "bigram fitness safe " {
    ...
    const s_eng = try allocator.dupe(u8, "Defend the east wall of the castle!");
    defer allocator.free(s_eng);

    // TODO: used ccrypt.cipher.caesar to encrypt the text
    const s_not = try allocator.dupe(u8, "Ghihqg wkh hdvw zdoo ri wkh fdvwoh!");
    defer allocator.free(s_not);
...
test "quadgram fitness" {
    ...
    const fit_eng = quad.calc(s_eng);
    const fit_not = quad.calc(s_not);
...
test "quadgram fitness safe" {
    ...
    // Check better than random text
    try std.testing.expect(quadFit(1).cmp(fit_eng, fit_not));
}
